...
public class Menu_Lv5 {
    Scanner scanner = new Scanner(System.in);
    //1. 속성
    private ArrayList<MenuItem_Lv5> menuItems = new ArrayList<>();    //  ArrayList<MenuItem> << 이게 타입 menuItems 이게 메뉴 라는 배열 생성 Lv5에 따라 private로 생성
    private String name; // 메뉴 클래스에서 사용 될 name 생성

    //2. 생성자가 name과 menuItems을 받음
    public Menu_Lv5(String name, ArrayList<MenuItem_Lv5> menuItems) {
        this.name = name;
    ...
    private void printCategory() {
        ...
        for (int i = 0; i < menuItems.size(); i++) { //각 종류의 메뉴이름,가격,코멘트 나열
            MenuItem_Lv5 menuItem = menuItems.get(i); // 밑에 코드를 사용하기 위해 초기화 함 해주고 (간접접근?) menuItems에 i번째 인덱스를 가져옴 하지만 for문이라 다 가져오는 것
            menuItem.getGettotalMenu(i + 1); // name + price + comment 가 있는 메뉴들을 나열

        }
    }
    public void getCategory() { //Lv5의 잔재물
        printCategory();
    ...
    private void displayMenu(int j) {
        //입력값은 받고 받은 값에 대한 메뉴를 보여줌 / 선택한 메뉴 보여주기 메서드 실행
            ...
            MenuItem_Lv5 menuItem = menuItems.get(j); //타입이 MenuItem이고 이름이 menuItem인 애는 menuItems 배열의 j번째 값을 가져온다.
            menuItem.getGetDisplay(); //가져온 값을 MenuItem클래스 31번째 메서드로 실행

        }



    public int getMenuItemsize() { //메뉴아이템즈 배열의 사이즈를 가져오는 메서드
        return menuItems.size();

    }
    public ArrayList<MenuItem_Lv5> getMenuItems() { //메뉴아이템즈르르 가져오는 메서드
        return menuItems;
    }
